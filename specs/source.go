...
type Source struct {
	...
	// DeterministicLokiID is a flag that indicates whether the source plugin should generate a random UUID as the value of _loki_id
	// or whether it should calculate a UUID that is a hash of the primary keys (if they exist) or the entire resource.
	DeterministicLokiID bool `json:"deterministic_loki_id,omitempty"`
}
...
func (s *Source) Validate() error {
	...
	if s.Path == "" {
		...
		if funk.ContainsString(officialPlugins, s.Name) {
			msg += fmt.Sprintf(". Hint: try setting path to loki/%s in your config", s.Name)
		}
		return fmt.Errorf(msg)
	}

	if len(s.Tables) == 0 {
		return fmt.Errorf("tables configuration is required. Hint: set the tables you want to sync by adding `tables: [...]` or use `loki tables` to list available tables")
	}
